...
func TestJulianFromTime(test *testing.T) {
	...
	t1 := time.Date(1877, time.August, 11, 7, 30, 0, 0, tzLocation)
	...
	t2 := time.Date(2018, time.June, 21, 0, 0, 0, 0, tzLocation)
	...
	t3 := time.Date(2016, time.January, 13, 0, 0, 0, 0, tzLocation)
	...
	t4 := time.Date(1978, time.January, 1, 0, 0, 0, 0, tzLocation)
	...
	t5 := time.Date(1978, time.July, 21, 15, 0, 0, 0, tzLocation)
	...
	t6 := time.Date(2018, time.December, 28, 0, 0, 0, 0, tzLocation)
	...
	t7 := time.Date(2000, time.January, 1, 12, 0, 0, 0, tzLocation)
...
func TestJulianFromTime_20000101(test *testing.T) {
	t1 := time.Date(2000, time.January, 1, 0, 0, 0, 0, tzLocation)
	res := JulianFromTime(t1)
...
func TestJulianFromTime_20180705(test *testing.T) {
	t1 := time.Date(2018, time.July, 5, 0, 0, 0, 0, tzLocation)
	res := JulianFromTime(t1)
